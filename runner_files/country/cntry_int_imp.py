...
class CntryIntImp(commodity_runner):
    def __init__(self, run_year: str, conn: Optional[connection]=None):
        super(CntryIntImp, self).__init__(conn)
    ...
    def run(self):
        ...
        source_tb = sql_vars['cntry_scr_tb_imp']
        output_tb = sql_vars['cntry_otpt_tb_imp']
        cntry_cd_mapping = sql_vars['cntry_cd_mapping']

        insert_sql = f"""
            SELECT 
                sr.country, sr.year, cntry_mp.cntry_cd as cntry_cd,
                replace(sr.imp_inr_cr, ',', '')::NUMERIC AS imp_inr_cr
            FROM {source_tb} sr
